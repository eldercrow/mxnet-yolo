...
def multibox_layer(from_layers, num_classes, sizes=[.2, .95],
    ...
    for k, from_layer in enumerate(from_layers):
        ...
        if not python_anchor:
            ...
            if dense_vh:
                ...
                num_anchors = len(ratios[k]) * len(sizes[k])
        ...
        kernel_loc = (1, 1)
        pad_loc = (0, 0)
        name_loc = '{}_loc_pred_conv'.format(from_name[1])

        num_loc_pred = num_anchors * 4
        bias = mx.symbol.Variable(name="{}_loc_pred_conv_bias".format(from_name[1]),
            init=mx.init.Constant(0.0), attr={'__lr_mult__': '2.0'})
        loc_pred_conv = mx.symbol.Convolution(data=from_layer[1], name=name_loc, bias=bias,
                num_filter=num_loc_pred, kernel=kernel_loc, pad=pad_loc)
        ...
        kernel_cls = (1, 1)
        pad_cls = (0, 0)
        name_cls = '{}_cls_pred_conv'.format(from_name[0])
        ...
        cls_pred_conv = mx.symbol.Convolution(data=from_layer[0], name=name_cls, bias=bias,
                num_filter=num_cls_pred, kernel=kernel_cls, pad=pad_cls)
        #
        # cls_pred_conv = mx.symbol.Convolution(data=from_layer[0], kernel=(1,1), \
        #     stride=(1,1), pad=(0,0), num_filter=num_cls_pred * upscale * upscale, \
        #     name="{}_cls_pred_conv".format(from_name[0]), no_bias=True)
        ...
        # #     name="{}_cls_pred_conv".format(from_name[0]), no_bias=True)
        #
        # beta = mx.symbol.Variable(name="{}_cls_pred_scale_beta".format(from_name[0]),
        #         init=FocalBiasInit(num_classes, 0.01))
        # cls_pred_conv = mx.sym.BatchNorm(cls_pred_conv, name='{}_cls_pred_scale'.format(from_name[0]),
        #         beta=beta, fix_gamma=False, use_global_stats=True)
        #
    ...
    if not python_anchor:
        anchor_boxes = mx.symbol.concat(*anchor_layers, dim=1)
